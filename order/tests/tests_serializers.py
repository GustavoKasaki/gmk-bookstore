...
def test_order_serializer():
    ...
    product1 = Product.objects.create(
        title="Test Product", description="Test Description", price=5.50, active=True
    )
    ...
    product1_data = next(
        product for product in data["product"] if product["title"] == "Test Product"
    )
    ...
    product2_data = next(
        product for product in data["product"] if product["title"] == "Test Product 2"
    )
